...
include_once('../../config/database.php');	
	// Thêm Sản Phẩm
		if(isset($_POST['xlthem'])){
			...
			$size=$_POST['size'];
			$mau=$_POST['mau'];
			if(isset($_POST['editor'])){$editor=$_POST['editor'];}else{$editor=NULL;}
     		 $sql_them=" INSERT INTO `sanpham`(`MaSP`, `TenSP`, `MaDM`, `MaNCC`, `MoTa`, `DonGia`, `MaAnh`, `AnhNen`) VALUES (NULL,'$tensp',$madm,$mancc,'$editor',$dongia,NULL,'$anhnen')";
	...
	//-----------------------------------------------------------------------------------------	
		...
		if(isset($_POST['xlsua'])){
			...
			if(isset($_POST['editor'])){$editor=$_POST['editor'];}else{$editor=NULL;}
			if($anhnen){
				$sql_them=" UPDATE `sanpham` SET `TenSP`='$tensp',`MaDM`='$madm',`MaNCC`='$mancc',`MoTa`='$editor',`DonGia`='$dongia',`AnhNen`='$anhnen' WHERE `MaSP`='$masp'";
			}else{
				$sql_them=" UPDATE `sanpham` SET `TenSP`='$tensp',`MaDM`='$madm',`MaNCC`='$mancc',`MoTa`='$editor',`DonGia`='$dongia' WHERE `MaSP`='$masp'";
			}
			$rs_them=mysqli_query($conn,$sql_them);
				if(isset($rs_them)){
