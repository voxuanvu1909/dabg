<?php
          if(isset($_GET['madm'])){
            ...
            $from =($trang-1)*12;
            $madm=$_GET['madm'];
            $dsb="SELECT * FROM `sanpham` WHERE `MaDM`=".$madm." ".$sx1." LIMIT $from,12 ";
            $dssp=mysqli_query($conn,$dsb);
            $so=mysqli_num_rows($dssp);

?>
<div class="container-fluid ">
	<div class="container">
		<div class="row boderbot" >
			<h3 class="col-md-10 ft"> <?php echo mb_strtoupper($t); ?> </h3>
		...
		<div class="row boderbot ">
			<?php while ($kq_dssp=mysqli_fetch_array($dssp)) {
...
<!-- Phân trang -->
    ...
    <?php  
            $ds_spn1b="SELECT MaSP FROM `sanpham` WHERE `MaDM`=".$madm;
      ...
      <ul class="pagination justify-content-center">
         <?php for($x=1;$x<=$sotrang;$x++){ ?>
          <li class="page-item">
          	  <a class="page-link" href="/dacs2/index.php?view=dksanpham&madm=<?php echo $madm;?>&t=<?php echo $t ?>&trang=<?php echo $x ?>&sx=<?php echo $sx1 ?>"><?php echo $x ;?></a></li>
          <?php } ?>


      </ul>
		<form action="" method="get" accept-charset="utf-8">
				<input hidden name="madm" value="<?php echo $madm ?>">
				<input hidden name="view" value="dksanpham">
				<input hidden name="t" value="<?php echo $t ?>">
			 <input type="submit" name="sx" id="result" hidden>
		</form>    
<?php       }?>
<!--theo tên hãng sản phẩm  -->
...
<?php
          if(isset($_GET['mancc'])){
            ...
            $from =($trang-1)*12;
              $mancc=$_GET['mancc'];
              $dsb="SELECT * FROM `sanpham` WHERE `MaNCC`=".$mancc." ".$sx1." LIMIT $from,12 ";
              $dssp=mysqli_query($conn,$dsb);
              $so=mysqli_num_rows($dssp);
?>
<div class="container-fluid ">
...
<!-- Phân trang -->
    ...
    <?php  
            $ds_spn1b="SELECT MaSP FROM `sanpham` WHERE `MaNCC`=".$mancc;
            $query_dssp2=mysqli_query($conn,$ds_spn1b);
            $sosp=mysqli_num_rows($query_dssp2);
            $sotrang = ceil($sosp/12); ?> </div>
      <hr>
      <ul class="pagination justify-content-center">
         <?php for($x=1;$x<=$sotrang;$x++){ ?>
          <li class="page-item">
          	  <a class="page-link" href="/dacs2/index.php?view=dksanpham&mancc=<?php echo $mancc;?>&t=<?php echo $t ?>&trang=<?php echo $x ?>&sx=<?php echo $sx1 ?>"><?php echo $x ;?></a></li>
          <?php } ?>
      </ul>
		<form action="" method="get" accept-charset="utf-8">
				<input hidden name="mancc" value="<?php echo $mancc ?>">
				<input hidden name="view" value="dksanpham">
				<input hidden name="t" value="<?php echo $t ?>">
			 <input type="submit" name="sx" id="result" hidden>
		</form>    
